...
PidParam_t NewPidParam, EepromPidParam;
...
#define EEPROM_ADDR_OFFSET_MODE 17
#define EEPROM_ADDR_OFFSET_OUTPUT  21
#define EEPROM_ADDR_OFFSET_OFFSET 25
...
#define DEF_MODE (int) 0 //Default PID Mode
#define DEF_OUTPUT 0.0f //Default PID Mode
...
void Init_EEPROM()
{
  ...
  if (EEPROM.begin(EEPROM_SIZE))
  {
    if (EEPROM.readByte(EEPROM_ADDR_OFFSET_INIT_FLAG))
    {
      ...
      EepromPidParam.Mode = EEPROM.readInt(EEPROM_ADDR_OFFSET_MODE);
      ...
      Serial.printf("   MODE [0x%02x] = %d\n", EEPROM_ADDR_OFFSET_MODE, EepromPidParam.Mode);
    }
    else
    {
...
//void SaveToEeprom(float_t Setpoint, float_t Kp, float_t Ki, float_t Kd, float_t Offset, int Mode) 
void SaveToEeprom()
{
  ...
  if (NewPidParam.Setpoint != EepromPidParam.Setpoint)
  {
    EepromPidParam.Setpoint = NewPidParam.Setpoint;
    EEPROM.writeFloat(EEPROM_ADDR_OFFSET_SP, NewPidParam.Setpoint);
    Serial.printf(("Setpoint, "));
    hasToBeSaved = true;
  }

  if (NewPidParam.Kp != EepromPidParam.Kp)
  {
    EepromPidParam.Kp = NewPidParam.Kp;
    EEPROM.writeFloat(EEPROM_ADDR_OFFSET_KP, NewPidParam.Kp);
    Serial.printf(("Kp, "));
    hasToBeSaved = true;
  }
  if (NewPidParam.Ki != EepromPidParam.Ki)
  {
    EepromPidParam.Ki = NewPidParam.Ki;
    EEPROM.writeFloat(EEPROM_ADDR_OFFSET_KI, NewPidParam.Ki);
    Serial.printf(("Ki, "));
    hasToBeSaved = true;
  }
  if (NewPidParam.Kd != EepromPidParam.Kd)
  {
    EepromPidParam.Kd = NewPidParam.Kd;
    EEPROM.writeFloat(EEPROM_ADDR_OFFSET_KD, NewPidParam.Kd);
    Serial.printf(("Kd, "));
    hasToBeSaved = true;
  }
  if (NewPidParam.Offset != EepromPidParam.Offset)
  {
    EepromPidParam.Offset = NewPidParam.Offset;
    EEPROM.writeFloat(EEPROM_ADDR_OFFSET_OFFSET, NewPidParam.Offset);
    Serial.printf(("Offset, "));
    hasToBeSaved = true;
  }
  if (NewPidParam.Mode != EepromPidParam.Mode)
  {
    EepromPidParam.Mode = NewPidParam.Mode;
    EEPROM.writeInt(EEPROM_ADDR_OFFSET_MODE, NewPidParam.Mode);
    Serial.printf(("Mode, "));
    hasToBeSaved = true;
  }
  if (hasToBeSaved)
  {
    ...
    EEPROM.commit();
    Serial.printf(("-> is stored in EEPROM\n"));
  }
  else
  {
    Serial.printf(("Already stored in EEPROM (NO CHANGE)\n"));
  }
}
...
String processor(const String &var)
{
  ...
  else if (var == "DEF_MODE")
  {
    return String(DEF_MODE, 10);
  }
  else if (var == "DEF_OUTPUT")
  {
    return String(DEF_OUTPUT, 1);
  }
  ...
  else if (var == "MODE")
  {
    ...
    int Mode = Pid_1.Mode;
    xSemaphoreGive(PID_1_Mutex);
    return String(Mode, 10);
  }
  else if (var == "OUTPUT")
  {
    xSemaphoreTake(PID_1_Mutex, portMAX_DELAY);
    float Output = Pid_1.Out;
    xSemaphoreGive(PID_1_Mutex);
...
void onUpload(AsyncWebServerRequest *request)
{
  if (request->hasParam("Setpoint") &&
      ...
      request->hasParam("Output") &&
      request->hasParam("Offset"))
  {
    NewPidParam.Setpoint = request->getParam("Setpoint")->value().toFloat();
    NewPidParam.Kp = request->getParam("Kp")->value().toFloat();
    NewPidParam.Ki = request->getParam("Ki")->value().toFloat();
    NewPidParam.Kd = request->getParam("Kd")->value().toFloat();
    NewPidParam.Mode = request->getParam("Mode")->value().toInt();
    NewPidParam.Output = request->getParam("Output")->value().toFloat();
    NewPidParam.Offset = request->getParam("Offset")->value().toFloat();

    xSemaphoreTake(PID_1_Mutex, portMAX_DELAY);
    Pid_1.Setpoint = NewPidParam.Setpoint;
    Pid_1.Kp = NewPidParam.Kp;
    Pid_1.Ki = NewPidParam.Ki;
    Pid_1.Kd = NewPidParam.Kd;
    Pid_1.Mode = NewPidParam.Mode;
    if (Pid_1.Mode == HEATING_MANUAL || Pid_1.Mode == COOLING_MANUAL) 
    {
      Pid_1.Out = NewPidParam.Output;
    }
    Offset_T = NewPidParam.Offset;
    Pid_1.Integrator = 0.0f;
    xSemaphoreGive(PID_1_Mutex);

    Serial.printf("...PID Parameters is set:\n");
    Serial.printf("...Sp = %.2f\n", NewPidParam.Setpoint);
    Serial.printf("...Kp = %.2f\n", NewPidParam.Kp);
    Serial.printf("...Ki = %.2f\n", NewPidParam.Ki);
    Serial.printf("...Kd = %.2f\n", NewPidParam.Kd);
    Serial.printf("...Mode = %d\n", NewPidParam.Mode);
...
/***********************************************************************************************/
void notifyClientsTemperarature(float Temperature)
{
...
void handleWebSocketMessage(void *arg, uint8_t *data, size_t len)
{
  ...
  if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT)
  {
    ...
    if(json.containsKey("param"))
    {
      ...
      NewPidParam.Setpoint = json["param"][0];
      NewPidParam.Kp = json["param"][1];
      NewPidParam.Ki = json["param"][2];
      NewPidParam.Kd = json["param"][3];
      NewPidParam.Mode = json["param"][4];
      NewPidParam.Output = json["param"][5];
      NewPidParam.Offset = json["param"][6];


      // float Kp = json["param"][1];
      // float Ki = json["param"][2];
      // float Kd = json["param"][3];
      // int Mode = json["param"][4];
      // float Output = json["param"][5];
      // float Offset = json["param"][6];

      int pressed_button = json["param"][7];
    ...
    else if (json.containsKey("action"))
    {
      ...
      switch (action)
      {
        case 1: {Q_01 = !Q_01; notifyClients(1); break;}
        case 2: {Q_02 = !Q_02; notifyClients(2); break;}
        case 3: {Q_03 = !Q_03; notifyClients(3); break;}
        case 4: {Q_04 = !Q_04; notifyClients(4); break;}
        case 5: {Q_05 = !Q_05; notifyClients(5); break;}
        case 6: {Q_06 = !Q_06; notifyClients(6); break;}
        case 7: {Q_07 = !Q_07; notifyClients(7); break;}
        case 8: {Q_08 = !Q_08; notifyClients(8); break;}
        default:break;
      }
    }
  }
...
void Init_PID_Parameters()
{
  if (EepromPidParam.initialized == 1)
  {
    ...
    Pid_1.Mode = EepromPidParam.Mode;
...
static void PID_Task(void *parameter)
{
  ...
  uint8_t notifyClientsTemperaratureCnt=0;
  ...
  for (;;)
  {
    ...
    if (++notifyClientsTemperaratureCnt >= 5 )
    {
      notifyClientsTemperarature(Pid_1.Measurement);
      notifyClientsTemperaratureCnt=0;
    }
