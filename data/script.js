...
//   let mode = Number.parseInt(Mode.value);
//   let output = Number.parseFloat(Output.value);
...
//   Output.value = output.toFixed(1);
//   Offset.value = offset.toFixed(1);
//   // then temperature display color may change...
//   setTemperature(temperature.innerText);

//   asyncAwaitRequest(`/upload?Setpoint=${sp}&Kp=${kp}&Ki=${ki}&Kd=${kd}&Mode=${mode}&Output=${output}&Offset=${offset}`);
// }
...
function onDefault(event) {
  ...
  Mode.value = Number.parseInt(Mode.dataset.max,10).toString(10);
  Output.value = Number.parseFloat(Output.dataset.max).toFixed(1);
...
function onUpload(event) {
  ...
  let ki = Number.parseFloat(Ki.value);
  let kd = Number.parseFloat(Kd.value);
  let mode = Number.parseInt(Mode.value,10);
  let output = Number.parseFloat(Output.value);
  let offset = Number.parseFloat(Offset.value);
  ...
  // then temperature display color may change...
  setTemperature(temperature.innerText);
  websocket.send(JSON.stringify({ "param":[sp,kp,ki,kd,mode,output,offset,button]}));
  //asyncAwaitRequest(`/upload?Setpoint=${sp}&Kp=${kp}&Ki=${ki}&Kd=${kd}&Mode=${mode}&Output=${output}&Offset=${offset}`);
}
...
function onSave(event) {
 ...
 // asyncAwaitRequest(`/save?Setpoint=${sp}&Kp=${kp}&Ki=${ki}&Kd=${kd}&Mode=${mode}&Output=${output}&Offset=${offset}`);
}
...
// ------------------------------------------------------------------
function setSetpointColor() {
  if (Mode.value == 0 || Mode.value == 1 || Mode.value == 2) {
    //panel.style.border= "1px solid #ff4c00"
    Setpoint.style.color = "#ff4c00";
    Mode.style.border = "0px solid black";
  }
  else if (Mode.value == 3 || Mode.value == 4 || Mode.value == 5) {
    //panel.style.border = "1px solid #0af"
    Setpoint.style.color = "#0af";
    Mode.style.border = "0px solid black";
  }
  else {
    Setpoint.style.color = "red";
...
try 
    {
      if ("temp" in data) {setTemperature(data.temp); }
      if ("status" in data) {
...
function onQ01(event) {
    websocket.send(JSON.stringify({'action':1}));
}
function onQ02(event) {
    websocket.send(JSON.stringify({'action':2}));
}
function onQ03(event) {
    websocket.send(JSON.stringify({'action':3}));
}
function onQ04(event) {
    websocket.send(JSON.stringify({'action':4}));
}
function onQ05(event) {
    websocket.send(JSON.stringify({'action':5}));
}
...
function onQ07(event) {
    websocket.send(JSON.stringify({'action':7}));
}
function onQ08(event) {
    websocket.send(JSON.stringify({'action':8}));
}
